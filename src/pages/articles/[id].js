    ...
    return (
      <>
        <section className={styles.hero}>
          <Image src={article.image.replace("localhost", "http://16.16.210.171")} width={800} height={484} layout='responsive' alt="hero-image" priority />
        ...
        <section className={styles.leftRightContainer}>
          {article.subheadings.map((item, index) => (
            <div className={`${styles.paraLeftRight} ${(index + 1) % 2 ? styles.leftCon : styles.rightCon}`}>
              <div className={styles.imgContainer}>
                <Image src={item.image.replace("localhost", "http://16.16.210.171")} width={640} height={488} layout='responsive' />
              </div>
